...
def run_dataflow_pipeline(input_file, output_file):
    ...
    service_account_email="dataflow-service-account@finalproject-1234567.iam.gserviceaccount.com",
    num_workers=10,  # Increase number of workers
    max_num_workers=20,  # Allow Dataflow to scale automatically
    worker_machine_type="n1-standard-2",  # Choose a more powerful machine type
    disk_size_gb=100  # Increase disk size
)


    # Ensure the input file path is in the correct format (GCS path)
    ...
    # Run the Apache Beam pipeline with Dataflow
    with beam.Pipeline(options=options) as p:
        (
            p
            | 'Read from GCS' >> beam.io.ReadFromText(
                f"gs://{bucket_name}/{file_name}",  # Use the full GCS path
                skip_header_lines=1,  # Skip the header line in the CSV
                coder=beam.coders.BytesCoder()  # Read raw bytes
            )
            | 'Decode Text' >> beam.ParDo(DecodeText(encoding))  # Decode using the detected encoding
            | 'Parse CSV' >> beam.Map(parse_csv_line)  # Parse CSV line into structured data
            | 'Filter Invalid Rows' >> beam.Filter(lambda x: x is not None)  # Filter out invalid rows
            | 'Preprocess Data' >> beam.ParDo(PreprocessData())  # Apply data preprocessing
            | 'Write to GCS' >> beam.io.WriteToText(output_file)  # Write the output to GCS
        )
...
def load_data_to_bigquery(df, dataset_id, table_id):
    try:
        ...
        dataset_ref = client.dataset(dataset_id, project="finalproject-1234567")
        try:
            client.get_dataset(dataset_ref)
        except Exception as e:
            logging.error(f"Dataset '{dataset_id}' not found in project 'finalproject-1234567'. Please create it first.")
            return
